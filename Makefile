...
main:
	g++ $(CFLAGS) $(SRCDIR)*.cpp $(DIR_JUEGO)*.cpp $(DIR_MATERIALES)*.cpp $(DIR_DATOS)*.cpp $(DIR_CONSTRUCCIONES)*.cpp $(DIR_CASILLEROS)*.cpp $(DIR_MENU)*.cpp $(DIR_OBJETIVOS)*.cpp $(DIR_ABB)*.cpp -g -o Andypolis
